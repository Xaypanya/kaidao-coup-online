...
function translatedAct(action){
    ...
    if(action === "foreign_aid"){
        return newActTranslated.foreign_aid;
    }
...
class CoupGame{
        ...
        this.players.map(x => {
            ...
            socket.on('g-challengeDecision', (res) => {
                ...
                if(bind.isChallengeBlockOpen) {
                    if(res.isChallenging) {
                        ...
                        bind.gameSocket.emit("g-addLog", `⚔️ ${res.challenger} ໄດ້ທ້າທາຍ ${res.challengee}`)
            ...
            socket.on('g-blockChallengeDecision', (res) => {
                ...
                if(bind.isChallengeBlockOpen) {
                    if(res.isChallenging) {
                        bind.closeChallenge();
                        bind.gameSocket.emit("g-addLog", `⚔️ ${res.challenger} ໄດ້ທ້າທາຍ ການສະກັດກັ້ນຂອງ ${res.challengee}`)
            ...
            socket.on('g-blockDecision', (res) => {
                ...
                if(bind.isChallengeBlockOpen) {
                    if(res.isBlocking) {
                        bind.closeChallenge();
                        bind.gameSocket.emit("g-addLog", `🛡️ ${res.blocker} ໄດ້ສະກັດກັ້ນ ${res.blockee}`)
                ...
                if(bind.isRevealOpen) {
                    ...
                    if(res.isBlock) { //block challenge (for example, a captain blocking a steal or a contessa blocking an assasinate)
                        if(res.revealedCard == res.counterAction.claim || (res.counterAction.counterAction == 'block_steal' && (res.revealedCard == 'ambassador' || res.revealedCard =='captain'))) { //challenge failed
                            bind.gameSocket.emit("g-addLog", `😭 ການທ້າທາຍຂອງ ${res.challenger}  ຕໍ່ກັບ ການສະກັດກັ້ນຂອງ ${res.challengee} ຫຼົ້ມເຫຼວ`)
                            ...
                            bind.gameSocket.emit("g-addLog", `🥳 ການທ້າທາຍຂອງ ${res.challenger}  ຕໍ່ກັບ ການສະກັດຂອງ ${res.challengee} ໄດ້ຮັບຜົນສຳເລັດ`)
                            bind.gameSocket.emit("g-addLog", `💥 ${res.challengee} ເສຍໄພ້ ${res.revealedCard}`)
                        ...
                        if(res.revealedCard == bind.actions[res.prevAction.action].influence) { // challenge failed
                            ...
                            bind.gameSocket.emit("g-addLog", `😭 ການທ້າທາຍຂອງ ${res.challenger} ຕໍ່ກັບ ${res.challengee} ຫຼົ້ມເຫຼວ`)
                            for(let i = 0; i < bind.players[challengeeIndex].influences.length; i++) { //revealed card needs to be replaced
                                if(bind.players[challengeeIndex].influences[i] == res.revealedCard) {
                                    bind.deck.push(bind.players[challengeeIndex].influences[i]);
                                    bind.deck = gameUtils.shuffleArray(bind.deck);
                            ...
                            bind.gameSocket.emit("g-addLog", `🥳 ການທ້າທາຍຂອງ ${res.challenger} ຕໍ່ກັບ ${res.challengee} ໄດ້ຮັບຜົນສຳເລັດ`)
                            bind.gameSocket.emit("g-addLog", `💥 ${res.challengee} ເສຍໄພ້ ${translatedCard(res.revealedCard)}`)
                            for(let i = 0; i < bind.players[challengeeIndex].influences.length; i++) { // 
                                if(bind.players[challengeeIndex].influences[i] == res.revealedCard) {
                                    bind.deck.push(bind.players[challengeeIndex].influences[i]);
                                    bind.deck = gameUtils.shuffleArray(bind.deck);
            ...
            socket.on('g-chooseInfluenceDecision', (res) => {
                ...
                if(bind.isChooseInfluenceOpen) {
                    bind.gameSocket.emit("g-addLog", `💥 ${res.playerName} ເສຍໄພ້ ${translatedCard(res.influence)}`)
    ...
    applyAction(action) {
        ...
        if(action.target) {
            logTarget = ` ກັບ ${action.target}`;
        }
        this.gameSocket.emit("g-addLog", `${action.source} ໄດ້ໃຊ້ ການ${translatedAct(action.action)}${logTarget}`)
    ...
    nextTurn() {
        ...
        this.players.forEach(x => {
            ...
            if(x.influences.length == 0 && !x.isDead) {// player is dead
                this.gameSocket.emit("g-addLog", `👻 ${x.name} ໄດ້ປະລາໄຊ ແລະ ອອກຈາກການແຂ່ງຂັນ! `)
                this.aliveCount-=1;
                x.isDead = true;
                x.money = 0;
            }
